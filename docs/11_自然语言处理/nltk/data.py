...
nltk_data_path = os.path.join(os.getcwd(), 'nltk', 'nltk_data')
path.append(nltk_data_path)
data_list = []
for i in os.listdir(nltk_data_path):
    if '.ipynb_checkpoints' in i: continue
    for j in os.listdir(os.path.join(nltk_data_path, i)):
        if '.ipynb_checkpoints' in j: continue
        data_list.append('%s/%s' %(i,j))
info={'nltk_data path':os.path.join('nltk', 'nltk_data'),
        'nltk_data included':data_list}
print('NLTK库信息：',info)
######################################################################
# Util Functions
######################################################################
